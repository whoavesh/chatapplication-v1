...
if (isset($_SESSION['unique_id'])) {
  ...
  if (mysqli_num_rows($query) > 0) {
    ...
    <div class="chat-message">
        <div class="flex items-end">
            <img src="php/images/pfp/' . htmlspecialchars($row['img']) . '" alt="My profile" class="w-6 h-6 rounded-2xl">
            <div class="flex flex-col space-y-2 text-xs max-w-xs mx-2 items-start">
                <div>
                    <span class="text-base font-normal px-4 py-2 rounded-2xl inline-block rounded-bl-none bg-gray-100 text-gray-800">'
                        . nl2br(htmlspecialchars($row['msg']), false) .
                    '</span>
                </div>';

                // Check if there's a file associated with the message
                if (!empty($row['file_path'])) {
                    // Determine the file type for rendering purposes
                    ...
                    if (in_array($fileExtension, ['jpg', 'jpeg', 'png', 'gif'])) {
                        ...
                        $output .= '<div class="mt-2">
                                        <img src="' . $filePath . '" alt="Uploaded image" class="w-32 h-auto rounded">
                                    </div>';
                    } else {
                        // Display a download link for other file types
                        $output .= '<div class="mt-2">
                ...
                $output .= '
                <span class="text-xs font-xs text-gray-400">'
